...
```
孙子（60%）+ 马可·奥勒留（40%）
  →  ConflictResolver → BehaviorIR → ClaudeRenderer  （inject 路径）
  →  FusionEngine → ContextBlock                      （text 路径）
  →  AI Agent
```
...
- **Character Pack** — 结构化 YAML 档案，涵盖思维方式、性格、行为、声音风格和来源；Schema 已扩展支持 Drive 对象、ToneAxes、decision_control、commitment_policy、evidence_level 等字段
- **融合引擎** — 支持加权混合（blend）、主导（dominant）、顺序（sequential）三种策略
- **Behavior IR** — 确定性冲突解析器，在渲染前生成类型化中间表示；inject 与 text 两条路径完全独立
...
```
sun-tzu/
├── meta.yaml          # 身份信息、类型、Schema 版本、许可证、可见性
├── mindset.yaml       # 核心原则、决策框架（heuristics 启发式规则、default_strategy、
│                      #   fallback_strategy、commitment_policy）、心智模型
├── personality.yaml   # 性格特质（+ confidence）、情绪倾向（+ baseline_mood、emotional_range、
│                      #   frustration_trigger、recovery_pattern）、Drive 对象、ConditionalSlot 字段
├── behavior.yaml      # 工作模式、decision_speed、decision_control、冲突风格、anti_patterns
├── voice.yaml         # 语气、tone_axes（正式度/温暖度/强度/幽默感）、词汇偏好、标志性短语
└── sources.yaml       # 来源材料引用（+ evidence_level：primary/secondary/tertiary）
```
...
默认 `--format inject` 经过 **Behavior IR 管线**处理：

```
CharacterPack(s)  →  ConflictResolver  →  BehaviorIR  →  ClaudeRenderer  →  系统提示词
```

解析器对每个行为槽位（communication style、leadership approach 等）应用确定性冲突策略，生成类型化的 `BehaviorIR`，再由渲染器输出最终文本。多角色冲突从不通过字符串拼接解决，始终可预测。

输出为**行为指令块** — 对 Agent 的具体行为指令，而非角色描述：
...
使用 `--format text` 可获得原有的角色描述格式（走 `FusionEngine → ContextBlock` 路径）。

### inject 路径的 `--explain` 输出

`--explain` 将结构化 YAML 摘要打印到 stderr，展示每个行为槽位的解析过程：
...
每个槽位展示哪个值获胜（`primary`）、是否检测到冲突（`has_conflict`），以及哪些值被丢弃及原因（`dropped`），使多角色冲突解析完全透明可审查。
...
| `--explain` | 关闭 | 将结构化 YAML 打印到 stderr（inject 路径输出 `slots`；text 路径输出 `merged`） |
